...
const uploadDetail = multer({
    storage: multer.diskStorage({
      ...
      filename(req, file, done) {
        ...
        done(null, path.basename(req.body.userid, ext) + Date.now() + ext);
...
app.post('/upload', uploadDetail.single('userfile'), function (req, res) {
    ...
    res.render('result', {id: `${req.body.userid}`, pw: `${req.body.userpw}`,
        username: `${req.body.username}`, userage: `${req.body.userage}`,
        img: `${req.file.path}`});
});
